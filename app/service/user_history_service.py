...
class UserHistoryService:
    ...
    def get_user_history(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            feedback_history = db_repository.get_user_feedback(
                self.db, user_id)

            # Fetch all activities for the user
            activity_history = db_repository.get_user_activities(
                self.db, user_id)
            ...
            logger.error(
                f"Error fetching history for user {user_id}: {str(e)}")
            raise
    ...
    def _calculate_mood_statistics(self, moods: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        most_common_mood = max(
            mood_distribution, key=mood_distribution.get) if mood_distribution else None
        least_common_mood = min(
            mood_distribution, key=mood_distribution.get) if mood_distribution else None
    ...
    async def _generate_mood_analysis(
        ...
        prompt = PERIODIC_MOOD_ANALYSIS_PROMPT.format(
            mood_data=formatted_mood_data)
